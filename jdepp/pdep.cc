...
namespace pdep
{
  ...
  bool chunk_t::setup(const dict_t *dict, const int next)
  {
    ...
    if ((_token_num = next - _mzero) == 0)
      return false;
    for (const token_t *m = mlast(); m >= mzero(); --m)
    {
      if (m->pos1() == dict->special)
      {
        if (m->pos2() == dict->comma)
          comma |= true;
        else if (m->pos2() == dict->period)
          period |= true;
        else if (m->pos2() == dict->bracket_in)
          ++bracket_in;
        else if (m->pos2() == dict->bracket_out)
          ++bracket_out;
      }
      else
      {
        if (_mtail == -1)
          _mtail = static_cast<int>(m - mzero());
        if (m->pos1() == dict->particle)
        {
          ...
          _mhead = static_cast<int>(m - mzero());
      }
    }
    if (head_id < 0 || head_id > id)
      return true;
#ifndef NDEBUG
  ...
  inline void parser::_add_particle_feature(const chunk_t *const bi, const chunk_t *const bj = 0)
  {
    ...
    if (bj)
      for (const chunk_t *bk = bi; bk != bj; ++bk)
      { // merge feature bits;
  ...
  {
    if (id >= 0)
      _fv.push_back(_fi + static_cast<ny::uint>(id));
  ...
  void parser::_event_gen_from_tuple(const int i)
  { // for chunking
    _fi = 1;
    _fv.clear();
  ...
  void parser::_parseChunking()
  {
    ...
    while (!_cinfo.empty())
    {
      ...
      if (_s->chunk(i)->head_id != -1)
        _cinfo.erase(it.base());
    }
  }
  template <const process_t MODE>
  void parser::_parseBackward()
  {
    ...
    for (int i = len - 2; i >= 0; --i)
    {
      ...
      for (int j = i + 1; j != -1; j = _s->chunk(j)->head_id)
      { // non-deterministic
  ...
  void parser::_parseTournament()
  {
    ...
    if (len < 2)
      return;
    if (MODE == LEARN)
    { // I don't merge the two outermost loops
      for (int i = 0; i < len - 2; ++i)
      {
        ...
        for (int j = i + 1; j <= len - 1; ++j)
        {
          ...
          {
            flag = true;
            _event_gen_from_tuple(i, j, h);
          }
          else if (j > h)
          {
            flag = false;
            _event_gen_from_tuple(i, h, j);
          }
          else
            continue;
#if defined(USE_OPAL) || defined(USE_MAXENT)
  ...
  void parser::_collectStat<CHUNK>()
  {
    ...
    for (int i = 1; i < _s->token_num; ++i)
    {
      ...
      if (m->chunk_start && m->chunk_start_gold)
      { // 'B' & 'B'
        if (prev)
          ++_chunk_stat.pp;
        else
          ++_chunk_stat.np, ++_chunk_stat.pn;
        prev = true;
      }
      else if (m->chunk_start || m->chunk_start_gold)
      { // 'B' & 'I' or 'I' & 'B'
        if (m->chunk_start)
          ++_chunk_stat.np;
        else
  ...
  template <>
  void parser::_collectStat<DEPND>()
  {
    if (_s->chunk_num >= 1)
    { // set >= 2 to ignore sentene w/ a single bunsetsu
      ++_depnd_stat.snum;
      _depnd_stat.bnum += _s->chunk_num - 1;
      int bcorr = 0;
      const int len = _s->chunk_num;
      for (int i = 0; i < len - 1; ++i)
        if (_s->chunk(i)->head_id == _s->chunk(i)->head_id_gold)
          ++bcorr;
    ...
    for (char *p(_s->postagged()), *q(p); p != r; p = ++q)
    {
      ...
      if (!_opt.ignore || std::strncmp(p, _opt.ignore, _opt.ignore_len) != 0)
      {
        ...
        case '#':
          break;
        case '*':
          _s->add_chunk(p, q - p, _s->token_num, true);
          break;
        default:
          _s->add_token(p, q - p, _dict);
          break;
  ...
  void parser::_batch()
  {
    ...
    if (_opt.input == RAW)
    {
      ...
      size_t read = 0;
      char buf[IOBUF_SIZE];
      std::setvbuf(stdin, &buf[0], _IOFBF, IOBUF_SIZE);
      while (1)
      {
        ...
        const size_t header_len = static_cast<size_t>(std::sprintf(header, "# S-ID: %ld; J.DepP\n", ++n));
    ...
    else
    {
      ...
      if (MODE != PARSE)
      {
        ...
        model += "/";
        model += input0[_opt.input];
        if (_opt.input == DEPND)
        {
          char sigparse[16];
          std::sprintf(sigparse, ".p%d", _opt.parser);
          model += sigparse;
        }
        if (MODE == LEARN)
        {
      ...
      while (1)
      {
        ...
        const bool input_ok = (avail = read(fd, q, static_cast<size_t>(q_end - q))) > 0;
        TIMER(if (MODE != LEARN) _io_t->stopTimer());
        if (!input_ok)
        { // output comments if found
          char *p = &buf[0];
          while (p != q)
          {
            char *p_end = p;
        ...
        while (1)
        {
          ...
          for (size_t i = 0; i < pos.size() - 1; ++i)
          {
            ...
            if (_opt.input == RAW)
              _s->add_token(line, read - 1, _dict);
            else
              switch (*line)
              {
              case '#':
                if (output)
                  _s->setHeader(line, read);
                break;
              case '*':
                if (_opt.input == DEPND)
  ...
  void parser::_learn()
  {
    ...
    case MAXENT:
      ...
      case SGD:
        _libme->use_SGD();
      case OWLQN:
        _libme->use_l1_regularizer(_maxent_opt.reg_cost);
        break;
      case LBFGS:
        _libme->use_l2_regularizer(_maxent_opt.reg_cost);
        break;
      default:
        my_errx(1, "%s", "MAXENT optimizer disabled.");
      }
      _libme->train();
                               ...
                               std::set<ny::uint> &particle_feature_ids)
  {
    ...
    {
      if (i == SURF)
        while (p != p_end && *p != SURFACE_END)
          ++p;
#ifdef USE_UNI_POS
      else if (i == POS1)
        while (p != p_end && *p != FEATURE_SEP && *p != FEATURE_SEP_UNI)
          ++p;
#endif
      else
        while (p != p_end && *p != FEATURE_SEP && *p != '\0')
          ++p;
#ifdef USE_UNI_POS
      ...
      *p = '\0';
      ...
      if (cs != p)
        ...
        if (i == YOMI || i == POS1 || i == POS2 || i == INFL)
        { //  || i > OTHER
#else
...
#endif
          case POS1:
            flag = std::strcmp(cs, post_particle) == 0;
            break;
          default:
            break;
          }
      ...
      if (flag)
        particle_feature_ids.insert(surf);
#ifdef USE_UNI_POS
      if (empty_POS2)
        --p;
#endif
  ...
  {
    if (_opt.verbose > 0)
      std::fprintf(stderr, "Loading dict..");
    ...
    if (!ioutil::FileExists(dict) && on_the_fly_train)
    {
      ...
      for (sbag_t::const_iterator it = sbag.begin(); it != sbag.end(); ++it)
      {
        ...
        const int index = static_cast<int>(std::distance(particle_feature_ids.begin(), jt));
  ...
  void parser::_setup_classifier(const input_t in, int argc, char **argv)
  {
    ...
    if (_opt.mode == BOTH)
    { // induce an appropriate classifier from input model
#ifdef USE_OPAL
    ...
    else
    {
      ...
      case '+':
        ...
        break;
      default:
        my_errx(1, "%s", "unknown model found");
      }
#ifndef USE_OPAL
      if (_opt.learner == OPAL)
  ...
  void parser::run()
  {
    ...
    // learn
    if (_opt.mode == LEARN || _opt.mode == BOTH)
    { // learn or both
      _setup_learner();
      _batch<LEARN>();
      _learn();
    ...
    if (_opt.mode != LEARN)
    {
      ...
      if (_opt.input != DEPND)
        _cleanup_classifier(CHUNK);
      if (_opt.input != CHUNK)
        _cleanup_classifier(DEPND);
    }
  }
}
// ToDo
